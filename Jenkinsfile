...
def IMAGE_TAG_WINDOWS = "itsrv80.nordicsemi.no/ncs-toolchain-windows:0.2.3"
def PLATFORM_LIST_WINDOWS  = ["nrf9160_pca10090ns", "nrf52840_pca10056"]
...
def check_and_store_sample(path, os, new_name) {
  script {
    if (fileExists(file_path)) {
      sh "cp ${path} artifacts/${os}/${new_name}"
    }
    else {
 ...
 // }
  ...
  stages {
    stage('Main') {
      parallel {
        stage('Linux') {
          agent {
              docker {
                image "$IMAGE_TAG"
                label "docker && ncs && build-node && linux"
          ...
          environment {
                ...
                SANITYCHECK_OPTIONS = "--board-root $WORKSPACE/nrf/boards --testcase-root $WORKSPACE/nrf/samples --testcase-root $WORKSPACE/nrf/applications --build-only --disable-unrecognized-section-test -t ci_build --inline-logs --platform nrf9160_pca10090ns"
      ...
      //              check_and_store_sample("$file_path", "linux", "nrf_desktop_${desktop_platforms[i]}.hex")
      //              file_path = "zephyr/sanity-out/${desktop_platforms[i]}/nrf_desktop/test_zrelease/zephyr/zephyr.hex"
      //              check_and_store_sample("$file_path", "linux", "nrf_desktop_${desktop_platforms[i]}_ZRelease.hex")
      //            }
      ...
      //              check_and_store_sample("$file_path", "linux", "${samples[i]}_nrf9160_pca10090.hex")
      //            }
      ...
      //              check_and_store_sample("$file_path", "linux", "${ns_samples[i]}_nrf9160_pca10090ns.hex")
      //            }
      ...
      //              check_and_store_sample("$file_path", "linux", "${ns_apps[i]}_nrf9160_pca10090ns.hex")
      //            }
      //          }
   ...
   //         }
        ...
        stage('Win10') {
          stages {
            stage('Compile Platforms') {
              steps {
                script {
                  def tasks  = [:]
                  for(int j=0; j< PLATFORM_LIST_WINDOWS.size(); j++) {
                    def PLATFORM = PLATFORM_LIST_WINDOWS[j]
                    tasks ["${PLATFORM}"] = {
                      node("ncs && build-node && windows && docker") {
                        stage('Compile'){
                          ...
                          script {
                            ...
                            echo bat(returnStdout: true, script: "docker run --rm -v ${WORKSPACE}\\${PLATFORM}:C:\\jenkins ${IMAGE_TAG_WINDOWS} cmd.exe /C call jenkins\\nrf\\scripts\\ci\\windows\\run.cmd ${PLATFORM}")
                          }

    //                      script {
    //                        /* Rename the nrf52 desktop samples */
    //                        //desktop_platforms = ['nrf52840_pca20041', 'nrf52_pca20037', 'nrf52840_pca10059']
    //                        for(int i=0; i<PLATFORM_LIST_WINDOWS.size(); i++) {
    //                          file_path = "zephyr/sanity-out/${PLATFORM_LIST_WINDOWS[i]}/nrf_desktop/test/zephyr/zephyr.hex"
    //                          check_and_store_sample("$file_path", "win", "nrf_desktop_${desktop_platforms[i]}.hex")
    //                          file_path = "zephyr/sanity-out/${PLATFORM_LIST_WINDOWS[i]}/nrf_desktop/test_zrelease/zephyr/zephyr.hex"
    //                          check_and_store_sample("$file_path", "win", "nrf_desktop_${desktop_platforms[i]}_ZRelease.hex")
    //                        }
    //                        /* Rename the nrf9160 samples */
    //                        samples = ['spm']
    //                        for(int i=0; i<samples.size(); i++)
    //                        {
    //                          file_path = "zephyr/sanity-out/nrf9160_pca10090/nrf9160/${samples[i]}/test_build/zephyr/zephyr.hex"
    //                          check_and_store_sample("$file_path", "win", "${samples[i]}_nrf9160_pca10090.hex")
    //                        }
    //                        ns_samples = ['lte_ble_gateway', 'at_client']
    //                        for(int i=0; i<ns_samples.size(); i++)
    //                        {
    //                          file_path = "zephyr/sanity-out/nrf9160_pca10090ns/nrf9160/${ns_samples[i]}/test_build/zephyr/zephyr.hex"
    //                          check_and_store_sample("$file_path", "win", "${ns_samples[i]}_nrf9160_pca10090ns.hex")
    //                        }
    //                        ns_apps = ['asset_tracker']
    //                        for(int i=0; i<ns_apps.size(); i++)
    //                        {
    //                          file_path = "zephyr/sanity-out/nrf9160_pca10090ns/${ns_apps[i]}/test_build/zephyr/zephyr.hex"
    //                          check_and_store_sample("$file_path", "win", "${ns_apps[i]}_nrf9160_pca10090ns.hex")
    //                        }
    //                      }
    //                      archiveArtifacts allowEmptyArchive: true, artifacts: 'artifacts/*.hex'
                          always {
                            // Clean up the working space at the end (including tracked files)
                            println("Skipping CleanWs")
                            //cleanWs()
                          }
                        }
                      }
